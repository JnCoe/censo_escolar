...
graf <- novo_cronograma %>%
    janitor::clean_names() %>%
    mutate(hibrido = as.numeric(hibrido), presencial = as.numeric(presencial)) %>% filter(rede == 'Municipal') %>%
    select(!rede) %>%
    googleVis::gvisBarChart(xvar='capital', options=list(bars='horizontal', isStacked=TRUE, height=1080))
...
graf <- novo_cronograma %>%
    janitor::clean_names() %>%
    mutate(hibrido = as.numeric(hibrido), presencial = as.numeric(presencial)) %>% filter(rede == 'Estadual') %>%
    select(!rede) %>%
    googleVis::gvisBarChart(xvar='capital', options=list(bars='horizontal', isStacked=TRUE, height=1080))
