...
Importante pontuar que há 13.726 pacientes vacinados que não possuem dados de sexo ou idade. Ainda assim, os dados calculados com base na RAIS parecem bastante alinhados. É notável que o número de vacinados na faixa até os 30 anos (no caso das mulehres) ou 40 anos (no caso dos homens) ultrapassa o total calculado pela RAIS. Como dito anteriormente, o cálculo está subestimado por não incluir auxiliares que prestam serviços para CNAEs diferentes dos selecionados. É possível também que uma série de preenchimentos incorretos dos dados tenham ocorrido.
...
### São Paulo - RAIS e vacinação

Para obter os dados por municípios, genero e idade, teremos que executar o código abaixo:
...
query <- "SELECT
  ...
  COUNT(DISTINCT
    CASE
      WHEN dose = '1ª Dose' THEN id_documento
  END
    ) AS prim_dose,
  COUNT(DISTINCT
    CASE
      WHEN dose = '2ª Dose' THEN id_documento
  END
    ) AS segun_dose,
  COUNT(DISTINCT
    CASE
      WHEN dose NOT IN ('1ª Dose', '2ª Dose') THEN id_documento
  END
    ) AS outras_doses,
...
A tabela final que possuímos é a seguinte:
```{r}
compilado_mun_det
```
