...
-----
Trecho anterior a ser excluído
...
Qual será a extensão desses casos atípicos? Vamos verificar manualmente:

```{r}
cnaes_cbo2 %>%
  group_by(ocupacao, nm_cnae) %>%
  summarise(total = sum(total)) %>%
  arrange(ocupacao) %>%
  DT::datatable(filter = 'top', options = list(scrollX='600px'))
```
...
cnaes_cbo2 %>%
  ...
  googleVis::gvisBarChart(xvar= "ocupacao", options=list(height=10000, isStacked='percent')) %>%
  plot()
```
...
Apesar de outros CNAEs possivelmente terem um grande volume de escolas, por não ser uma abrangência absoluta, iremos coletar apenas os vínculos dessas instituições com base nos CBOs. O problema são os CBOs com alta participação de "Administração pública em geral", pois não sabemos do que se trata. Por exemplo, pedagogos nessa categoria podem estar contratadosOs CBOS selecionados:

-----

O processo vai ser assim:

etapa 1: filtragem de CNAEs relevantes (incluindo terceirizados, excluindo ensino superior por ex)

etapa 2: Seleção de CNAEs em que todos os vínculos serão obtidos (ensino fundamental)

etapa 3: Seleção de CNAEs em que apenas alguns vinculos serão mantidos com base no CBO

Vamos fazer duas contagens:
educação basica
Educação geral

Incluir
('239405', '334110', '231205', '239210', '239220', '231305', '262830', '231315', '231320', '232135', '231325', '232140', '232150', '234624', '234628', '234652', '232155', '331205', '231210', '231110', '232160', '232165', '251505') 
